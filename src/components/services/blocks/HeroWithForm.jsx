        ...
        <div className="grid items-center gap-8 md:grid-cols-2">
          <div>
            ...
            <div className="flex flex-wrap gap-3">
              {ctas.map((cta, i) => (
                <a key={i} href={cta.href || '#contact'} className={`h-12 rounded-xl px-6 font-semibold inline-flex items-center justify-center ${
                  cta.primary ? 'bg-white text-blue-900' : 'border-white/40 bg-white/10 text-white backdrop-blur-sm border'
                }`}>
                  {cta.label}
                </a>
              ))}
